...
#include "c1_inverse_kinematics.h"
...
static void initialize_c1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance);
static void initialize_params_c1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance);
static void enable_c1_inverse_kinematics(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance);
static void disable_c1_inverse_kinematics(SFc1_inverse_kinematicsInstanceStruct *
  chartInstance);
static void c1_update_debugger_state_c1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance);
static const mxArray *get_sim_state_c1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance);
static void set_sim_state_c1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance, const mxArray *c1_st);
static void finalize_c1_inverse_kinematics(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance);
static void sf_c1_inverse_kinematics(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance);
static void initSimStructsc1_inverse_kinematics
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance);
static void init_script_number_translation(uint32_T c1_machineNumber, uint32_T
  c1_chartNumber);
static const mxArray *c1_sf_marshallOut(void *chartInstanceVoid, void *c1_inData);
static void c1_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance, const mxArray *c1_I, const char_T *c1_identifier, real_T c1_y
  [144]);
static void c1_b_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
...
static const mxArray *c1_b_sf_marshallOut(void *chartInstanceVoid, void
  *c1_inData);
static real_T c1_c_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance, const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
...
static const mxArray *c1_c_sf_marshallOut(void *chartInstanceVoid, void
  *c1_inData);
static int32_T c1_d_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance, const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
static void c1_c_sf_marshallIn(void *chartInstanceVoid, const mxArray
  *c1_mxArrayInData, const char_T *c1_varName, void *c1_outData);
static uint8_T c1_e_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance, const mxArray *c1_b_is_active_c1_inverse_kinematics, const
  char_T *c1_identifier);
static uint8_T c1_f_emlrt_marshallIn(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance, const mxArray *c1_u, const emlrtMsgIdentifier *c1_parentId);
static void init_dsm_address_info(SFc1_inverse_kinematicsInstanceStruct
  *chartInstance);
  ...
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance)
{
  ...
  chartInstance->c1_is_active_c1_inverse_kinematics = 0U;
}
  ...
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance)
{
  ...
  c1_hoistedGlobal = chartInstance->c1_is_active_c1_inverse_kinematics;
  ...
  (SFc1_inverse_kinematicsInstanceStruct *chartInstance, const mxArray *c1_st)
{
  ...
  chartInstance->c1_is_active_c1_inverse_kinematics = c1_e_emlrt_marshallIn
    (chartInstance, sf_mex_dup(sf_mex_getcell(c1_u, 1)),
     "is_active_c1_inverse_kinematics");
  sf_mex_destroy(&c1_u);
  c1_update_debugger_state_c1_inverse_kinematics(chartInstance);
...
static const mxArray *c1_sf_marshallOut(void *chartInstanceVoid, void *c1_inData)
{
  ...
  const mxArray *c1_y = NULL;
  SFc1_inverse_kinematicsInstanceStruct *chartInstance;
  chartInstance = (SFc1_inverse_kinematicsInstanceStruct *)chartInstanceVoid;
...
const mxArray *sf_c1_inverse_kinematics_get_eml_resolved_functions_info(void)
{
  ...
  char_T *c1_identifier)
{
  ...
  c1_y = c1_f_emlrt_marshallIn(chartInstance, sf_mex_dup
    (c1_b_is_active_c1_inverse_kinematics), &c1_thisId);
  sf_mex_destroy(&c1_b_is_active_c1_inverse_kinematics);
  return c1_y;
}
...
/* SFunction Glue Code */
void sf_c1_inverse_kinematics_get_check_sum(mxArray *plhs[])
{
...
mxArray *sf_c1_inverse_kinematics_get_autoinheritance_info(void)
{
...
static const mxArray *sf_get_sim_state_info_c1_inverse_kinematics(void)
{
  const char *infoFields[] = { "chartChecksum", "varInfo" };

  mxArray *mxInfo = mxCreateStructMatrix(1, 1, 2, infoFields);
  const char *infoEncStr[] = {
    "100 S1x2'type','srcId','name','auxInfo'{{M[1],M[5],T\"I\",},{M[8],M[0],T\"is_active_c1_inverse_kinematics\",}}"
  };

  mxArray *mxVarInfo = sf_mex_decode_encoded_mx_struct_array(infoEncStr, 2, 10);
  mxArray *mxChecksum = mxCreateDoubleMatrix(1, 4, mxREAL);
  sf_c1_inverse_kinematics_get_check_sum(&mxChecksum);
  ...
  fullDebuggerInitialization)
{
  if (!sim_mode_is_rtw_gen(S)) {
    SFc1_inverse_kinematicsInstanceStruct *chartInstance;
    chartInstance = (SFc1_inverse_kinematicsInstanceStruct *) ((ChartInfoStruct *)
...
static void sf_opaque_initialize_c1_inverse_kinematics(void *chartInstanceVar)
{
  chart_debug_initialization(((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar)->S,0);
  initialize_params_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
  initialize_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_enable_c1_inverse_kinematics(void *chartInstanceVar)
{
  enable_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_disable_c1_inverse_kinematics(void *chartInstanceVar)
{
  disable_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
}

static void sf_opaque_gateway_c1_inverse_kinematics(void *chartInstanceVar)
{
  sf_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
}

extern const mxArray* sf_internal_get_sim_state_c1_inverse_kinematics(SimStruct*
  S)
{
  ...
  prhs[2] = (mxArray*) get_sim_state_c1_inverse_kinematics
    ((SFc1_inverse_kinematicsInstanceStruct*)chartInfo->chartInstance);/* raw sim ctx */
  prhs[3] = (mxArray*) sf_get_sim_state_info_c1_inverse_kinematics();/* state var info */
...
extern void sf_internal_set_sim_state_c1_inverse_kinematics(SimStruct* S, const
  mxArray *st)
{
  ChartInfoStruct *chartInfo = (ChartInfoStruct*) ssGetUserData(S);
  mxArray *plhs[1] = { NULL };
  ...
  set_sim_state_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInfo->chartInstance, mxDuplicateArray(plhs[0]));
  mxDestroyArray(plhs[0]);
}

static const mxArray* sf_opaque_get_sim_state_c1_inverse_kinematics(SimStruct* S)
{
  return sf_internal_get_sim_state_c1_inverse_kinematics(S);
}

static void sf_opaque_set_sim_state_c1_inverse_kinematics(SimStruct* S, const
  mxArray *st)
{
  sf_internal_set_sim_state_c1_inverse_kinematics(S, st);
}

static void sf_opaque_terminate_c1_inverse_kinematics(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    SimStruct *S = ((SFc1_inverse_kinematicsInstanceStruct*) chartInstanceVar)
      ->S;
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      sf_clear_rtw_identifier(S);
    }

    finalize_c1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
      chartInstanceVar);
...
{
  initSimStructsc1_inverse_kinematics((SFc1_inverse_kinematicsInstanceStruct*)
    chartInstanceVar);
}

extern unsigned int sf_machine_global_initializer_called(void);
static void mdlProcessParameters_c1_inverse_kinematics(SimStruct *S)
{
  int i;
  ...
  if (sf_machine_global_initializer_called()) {
    initialize_params_c1_inverse_kinematics
      ((SFc1_inverse_kinematicsInstanceStruct*)(((ChartInfoStruct *)
         ssGetUserData(S))->chartInstance));
  }
}

static void mdlSetWorkWidths_c1_inverse_kinematics(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
...
static void mdlRTW_c1_inverse_kinematics(SimStruct *S)
{
  if (sim_mode_is_rtw_gen(S)) {
...
static void mdlStart_c1_inverse_kinematics(SimStruct *S)
{
  SFc1_inverse_kinematicsInstanceStruct *chartInstance;
  chartInstance = (SFc1_inverse_kinematicsInstanceStruct *)malloc(sizeof
    (SFc1_inverse_kinematicsInstanceStruct));
  memset(chartInstance, 0, sizeof(SFc1_inverse_kinematicsInstanceStruct));
  ...
  chartInstance->chartInfo.sFunctionGateway =
    sf_opaque_gateway_c1_inverse_kinematics;
  chartInstance->chartInfo.initializeChart =
    sf_opaque_initialize_c1_inverse_kinematics;
  chartInstance->chartInfo.terminateChart =
    sf_opaque_terminate_c1_inverse_kinematics;
  chartInstance->chartInfo.enableChart = sf_opaque_enable_c1_inverse_kinematics;
  chartInstance->chartInfo.disableChart =
    sf_opaque_disable_c1_inverse_kinematics;
  chartInstance->chartInfo.getSimState =
    sf_opaque_get_sim_state_c1_inverse_kinematics;
  chartInstance->chartInfo.setSimState =
    sf_opaque_set_sim_state_c1_inverse_kinematics;
  chartInstance->chartInfo.getSimStateInfo =
    sf_get_sim_state_info_c1_inverse_kinematics;
  chartInstance->chartInfo.zeroCrossings = NULL;
  chartInstance->chartInfo.outputs = NULL;
  chartInstance->chartInfo.derivatives = NULL;
  chartInstance->chartInfo.mdlRTW = mdlRTW_c1_inverse_kinematics;
  chartInstance->chartInfo.mdlStart = mdlStart_c1_inverse_kinematics;
  chartInstance->chartInfo.mdlSetWorkWidths =
    mdlSetWorkWidths_c1_inverse_kinematics;
...
void c1_inverse_kinematics_method_dispatcher(SimStruct *S, int_T method, void
  *data)
{
  switch (method) {
   case SS_CALL_MDL_START:
    mdlStart_c1_inverse_kinematics(S);
    break;

   case SS_CALL_MDL_SET_WORK_WIDTHS:
    mdlSetWorkWidths_c1_inverse_kinematics(S);
    break;

   case SS_CALL_MDL_PROCESS_PARAMETERS:
    mdlProcessParameters_c1_inverse_kinematics(S);
    break;

   default:
    /* Unhandled method */
    sf_mex_error_message("Stateflow Internal Error:\n"
                         "Error calling c1_inverse_kinematics_method_dispatcher.\n"
