...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_state_process_check_sum_call( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  extern unsigned int sf_rov_design_analysis_process_check_sum_call( int nlhs,
   mxArray * plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_state_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  if(sf_rov_design_analysis_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return
  1;
  return 0;
}
...
{
  extern unsigned int sf_state_autoinheritance_info( int nlhs, mxArray * plhs[],
   int nrhs, const mxArray * prhs[] );
  if(sf_state_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
...
extern unsigned int sf_state_method_dispatcher(SimStruct *S, const char
 *chartName, int_T method, void *data);
extern unsigned int sf_rov_design_analysis_method_dispatcher(SimStruct *S, const
 char *chartName, int_T method, void *data);
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
 char *chartName, int_T method, void *data)
{
  if(sf_state_method_dispatcher(simstructPtr,chartName,method,data)) return 1;
  if(sf_rov_design_analysis_method_dispatcher(simstructPtr,chartName,method,data))
  return 1;
  return 0;
}
extern void state_terminator(void);
extern void rov_design_analysis_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    state_terminator();
    rov_design_analysis_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void state_initializer(void);
extern void rov_design_analysis_initializer(void);
extern void state_debug_initialize(void);
extern void rov_design_analysis_debug_initialize(void);
void sf_machine_global_initializer(void)
{
  if(sfMachineGlobalInitializerCallable) {
    ...
    state_debug_initialize();
    state_initializer();
    rov_design_analysis_debug_initialize();
    rov_design_analysis_initializer();
  }
  return;
}
